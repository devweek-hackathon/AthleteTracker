...
class Map extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      this.props.raceStarted &&
      this.props.raceStarted !== prevProps.raceStarted
    ) {
      console.log('FROM THE MAP, RACE STARTED', this.props)
    }

    if (!_.isEqual(this.props.newPoint, prevProps.newPoint)) {
      const {lat, lng, racerId, radius} = this.props.newPoint
      if (this.props.lat) {
        this.map.addObject(
          new window.H.map.Circle({lat, lng}, radius * 50, {
            style: {
              strokeColor: 'rgba(55, 85, 170, 0.6)', // Color of the perimeter
              lineWidth: 2,
              fillColor: colors[(racerId - 1) % colors.length] // Color of the circle
            }
          })
        )
      }
